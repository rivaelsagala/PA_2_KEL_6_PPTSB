...
class SektorController extends Controller
{
    ...
    public function sektor_dashboard()
    {
        ...
        return view('sektor/index', compact('totalregional', 'totalpusat', 'totalcabang', 'totalsektor',));
    }
    ...
    public function pengumuman()
    {
        ...
        return view('/sektor/pengumuman/index', compact('anggota', 'kegiatan', 'totalData'));
    }
    ...
    public function tambah_pengumuman(Request $request)
    {
        ...
        if ($request->hasFile('gambar')) {
            ...
            return redirect('/sektor-pengumuman')->with('toast_success', 'Data berhasil ditambah');
        } else {
            return redirect('/sektor-pengumuman')->with('toast_error', 'Foto tidak ditemukan');
        }
    }
    ...
    public function edit_pengumuman_halaman($id)
    {
        ...
        $anggota = Anggota::all();
        return view('sektor/pengumuman/edit', compact('kegiatan', 'anggota'));
    }
    ...
    public function edit_pengumuman(Request $request, $id)
    {
        ...
        try {
            ...
            DB::commit();
            return redirect('/sektor-pengumuman')->with('toast_success', 'Data pengumuman berhasil diperbarui');
        } catch (\Exception $e) {
            DB::rollBack();
            return redirect('/sektor-pengumuman')->with('toast_error', 'Gagal memperbarui data: ' . $e->getMessage());
        }
    }
    ...
    public function delete_pengumuman($id)
    {
        ...
        if ($delete) {
            return redirect('/sektor-pengumuman');
        } else {
            return redirect('/sektor-pengumuman')->with(['error' => "Gagal Di Hapus!"]);
        }
    }
    ...
    public function kepala_keluarga()
    {
        ...
        return view('/sektor/kepala_keluarga/index', compact('totalData', 'anggota', 'pekerjaans', 'pendidikans', 'sektors', 'statuses'));
    }
    ...
    public function tambah_kepala_keluarga(Request $request)
    {
        ...
        if ($request->hasFile('foto')) {
            ...
            return redirect('/sektor-anggota')->with('toast_error', 'Foto tidak ditemukan');
        }
        ...
        try {
            ...
            User::create([
                ...
                'password' => $registrasi->no_registrasi,
                'kode' => $registrasi->no_registrasi,
                'role' => 'Kepala Keluarga',
            ]);

            DB::commit();
            return redirect('/sektor-kepala_keluarga')->with('toast_success', 'Data berhasil ditambah');
        } catch (\Exception $e) {
            DB::rollBack();
            return redirect('/sektor-kepala_keluarga')->with('toast_error', 'Gagal menambah data: ' . $e->getMessage());
        }
    }
    ...
    public function edit_kepala_keluarga_show($id)
    {
        ...
        return view('/sektor/kepala_keluarga/edit', compact('anggota', 'statuses', 'pekerjaans', 'pendidikans', 'hubunganKeluargas'));
    }
    ...
    public function edit_kepala_keluarga(Request $request, $id)
    {
        ...
        try {
            ...
            if ($sektor) {
                return redirect()->route('sektor_sektor.detail', ['id' => $sektor->id_sektor])->with('toast_success', 'Data kepala keluarga berhasil diperbarui');
            } else {
                return redirect()->route('error.page')->with('toast_error', 'Sektor tidak ditemukan.');
            }
        } catch (\Exception $e) {
            DB::rollBack();
            $sektor = Sektor::where('kode_sektor', auth()->user()->kode)->first();
            if ($sektor) {
                return redirect()->route('sektor_sektor.detail', ['id' => $sektor->id_sektor])->with('toast_error', 'Data kepala keluarga gagal diperbarui');
            } else {
                return redirect()->route('error.page')->with('toast_error', 'Sektor tidak ditemukan.');
            }
        }
    ...
    public function detail_kepala_keluarga($id)
    {
        ...
        return view('/sektor/kepala_keluarga/detail', compact('pekerjaans', 'statuses', 'pekerjaans', 'pendidikans', 'kepala_keluarga', 'anggota', 'data', 'sektors', 'registrasi_id'));
    }
    ...
    public function tambah_anggota(Request $request)
    {
        ...
        try {
            ...
            $sektor = Sektor::where('kode_sektor', auth()->user()->kode)->first();
            if ($sektor) {
                return redirect()->route('sektor_sektor.detail', ['id' => $sektor->id_sektor])->with('toast_success', 'Data Anggota berhasil ditambah');
            } else {
                return redirect()->route('error.page')->with('toast_error', 'Sektor tidak ditemukan.');
            }
        } catch (\Exception $e) {
            ...
            $sektor = Sektor::where('kode_sektor', auth()->user()->kode)->first();
            if ($sektor) {
                return redirect()->route('sektor_sektor.detail', ['id' => $sektor->id_sektor])->with('toast_error', 'Sektor tidak ditemukan.');
            } else {
                return redirect()->route('error.page')->with('toast_error', 'Sektor tidak ditemukan.');
            }
        }
    ...
    public function edit_anggota_show($id)
    {
        ...
        return view('/sektor/anggota/edit', compact('anggota', 'statuses', 'pekerjaans', 'pendidikans', 'hubunganKeluargas', 'sektors'));
    }
    ...
    public function edit_anggota(Request $request, $id)
    {
        ...
        try {
            ...
            $sektor = Sektor::where('kode_sektor', auth()->user()->kode)->first();
            if ($sektor) {
                return redirect()->route('sektor_sektor.detail', ['id' => $sektor->id_sektor])->with('toast_success', 'Data Anggota berhasil diperbarui');
            } else {
                return redirect()->route('error.page')->with('toast_error', 'Sektor tidak ditemukan.');
            }
        } catch (\Exception $e) {
            ...
            $sektor = Sektor::where('kode_sektor', auth()->user()->kode)->first();
            if ($sektor) {
                return redirect()->route('sektor_sektor.detail', ['id' => $sektor->id_sektor])->with('toast_error', 'Gagal memperbarui data: ' . $e->getMessage());
            } else {
                return redirect()->route('error.page')->with('toast_error', 'Sektor tidak ditemukan.');
            }
        }
    ...
    public function sektor()
    {
        ...
        return view('sektor/sektor/index', compact('data', 'totalData', 'cabang', 'anggota', 'pindah_sektor', 'totalPindah'));
    }
    ...
    public function edit_sektor_show($id)
    {
        ...
        return view('sektor.sektor.edit', compact('sektor'));
    }
    ...
    public function edit_sektor(Request $request, $id)
    {
        ...
        if ($sektor) {
            ...
            $sektors = Sektor::where('kode_sektor', auth()->user()->kode)->first();
            if ($sektors) {
                return redirect()->route('sektor_sektor.detail', ['id' => $sektor->id_sektor])->with('toast_success', 'Data sektor berhasil diperbarui');
            }
        } else {
            return redirect('/sektor-sektor')->with('toast_error', 'Data tidak ditemukan');
        }
    }
    ...
    public function tambah_sektor(Request $request)
    {
        ...
        return redirect('/sektor-sektor')->with('toast_success', 'Data berhasil ditambah');
    }

    // $sektor = Sektor::where('kode_sektor', $user->kode)->first();

    // if ($sektor) {
    //     return redirect()->route('sektor.detail', ['id' => $sektor->id_sektor]);
    // } else {
    //     return redirect()->route('login.form')->withErrors('Sektor tidak ditemukan.');
    // }
    ...
    public function detail_sektor($id)
    {
        ...
        return view('/sektor/sektor/detail', compact('current_sektor', 'totalData', 'anggota', 'sektores', 'pendidikans', 'pekerjaans', 'sektors', 'statuses'));
    }
    ...
    public function pejabat()
    {
        ...
        return view('/sektor/pejabat_struktural/index', compact('pejabat', 'totalData', 'statuses', 'bidangs', 'jabatans', 'anggotas'));
    }
    ...
    public function tambah_pejabat(Request $request)
    {
        ...
        return redirect('/sektor-pejabat')->with('toast_success', 'Data berhasil ditambah');
    }

    public function edit_pejabat_show($id)
    {
        $pejabat = PejabatStruktural::where('id_pejabat_struktural', $id)->first();
        return view('sektor.pejabat_struktural.edit', compact('pejabat'));
    }
    ...
    public function edit_pejabat(Request $request, $id)
    {
        ...
        if ($pejabat) {
            ...
            // dd($regional);
            $pejabat->save();
            return redirect('/sektor-pejabat')->with('toast_success', 'Data berhasil diedit');
        } else {
            return redirect('/sektor-pejabat')->with('toast_error', 'Data tidak ditemukan');
        }
    }
}
