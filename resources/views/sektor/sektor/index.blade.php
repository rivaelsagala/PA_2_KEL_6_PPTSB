...
<div class="card" id="employeeTable">
    <div class="card-body">
        <div class="flex items-center gap-3 mb-4">
            <h6 class="text-15 grow">Data Sektor (<b class="total-Employs">{{$totalData}}</b>)</h6>
            <div class="shrink-0">
                    ...
                    <td
                        ...
                        <div class="flex gap-3">
                            <a class="flex items-center justify-center transition-all duration-200 ease-linear rounded-md size-8 bg-slate-100 text-slate-500 hover:text-custom-500 hover:bg-custom-100 dark:bg-zink-600 dark:text-zink-200 dark:hover:bg-custom-500/20 dark:hover:text-custom-500"
                                href="/sektor-detail/{{ $d->id_sektor }}"><i data-lucide="eye"
                                    class="inline-block size-3"></i> </a>
                            <a href="/sektor-edit-sektor/{{ $d->id_sektor }}"
...
<div class="card" id="employeeTable">
    <div class="card-body">
        <div class="flex items-center gap-3 mb-4">
            <h6 class="text-15 grow">Permintaan Pindah Sektor (<b class="total-Employs">{{$totalPindah}}</b>)</h6>
        </div>
        ...
        <table id="rowBorder" style="width:100%">
            ...
            <tbody class="listt" id="employeeList">
                @foreach ($pindah_sektor as $d)
                <tr>
                    <td class="px-3.5 py-2.5 first:pl-5 last:pr-5 border-y border-slate-200 dark:border-zink-500 ID">
                        <a href="#!"
                            class="transition-all duration-150 ease-linear text-custom-500 hover:text-custom-600">#RP{{$d->id_reg_pindah}}</a>
                    </td>
                    <td class="px-3.5 py-2.5 first:pl-5 last:pr-5 border-y border-slate-200 dark:border-zink-500 Name">
                        <a href="#!" class="flex items-center gap-3">
                            <h5 class="grow">{{$d->no_pindah}}</h5>
                        </a>
                    </td>
                    ...
                    <td class="px-3.5 py-2.5 first:pl-5 last:pr-5 border-y border-slate-200 dark:border-zink-500 Phone">
                        <img src="/assets/gambar/{{ $d->file_pindah }}" style="width:50px">
                    </td>
                    <td
                        class="px-3.5 py-2.5 first:pl-5 last:pr-5 border-y border-slate-200 dark:border-zink-500 Country">
                        {{$d->sektor_tujuan}}
                    ...
                    <td
                        class="px-3.5 py-2.5 first:pl-5 last:pr-5 border-y border-slate-200 dark:border-zink-500 Action">
                        <div class="flex gap-3">
                            <form id="deleteForm_{{ $d->id_reg_pindah }}"
                                action="{{ route('sektor.tolak', $d->id_reg_pindah) }}" method="POST">
                                @csrf
                                @method('POST')
                                <button type="submit"
                                    class="toggle-status flex items-center justify-center text-white  transition-all duration-200 ease-linear rounded-md size-8 hover:text-white bg-red-500">
                                    <i data-lucide="x" class="size-4"></i> <!-- Icon untuk aktifkan -->
                                </button>
                            </form>
                            <form id="deleteForm_{{ $d->id_reg_pindah }}"
                                action="{{ route('sektor.terima', $d->id_reg_pindah) }}" method="POST">
                                @csrf
                                @method('POST')
                                <button type="submit"
                                    class="toggle-status flex items-center justify-center text-white  transition-all duration-200 ease-linear rounded-md size-8 hover:text-white bg-green-500">
...
<div id="addEmployeeModal" modal-center=""
    ...
    <div class="w-screen md:w-[30rem] bg-white shadow rounded-md dark:bg-zink-600">
        <div class="flex items-center justify-between p-4 border-b dark:border-zink-500">
            <h5 class="text-16" id="addEmployeeLabel">Tambah Sektor</h5>
        ...
        <div class="max-h-[calc(theme('height.screen')_-_180px)] p-4 overflow-y-auto">
            <form class="create-form" id="create-form" action="/tambah-sektor" method="POST"
                ...
                <div class="grid grid-cols-1 gap-4 xl:grid-cols-12">
                    <div class="xl:col-span-12">
                        <label for="kode_sektor" class="inline-block mb-2 text-base font-medium">Kode Sektor</label>
                        <input type="text" id="kode_sektor" name="kode_sektor"
                            class="form-input border-slate-200 dark:border-zink-500 focus:outline-none focus:border-custom-500 disabled:bg-slate-100 dark:disabled:bg-zink-600 disabled:border-slate-300 dark:disabled:border-zink-500 dark:disabled:text-zink-200 disabled:text-slate-500 dark:text-zink-100 dark:bg-zink-700 dark:focus:border-custom-800 placeholder:text-slate-400 dark:placeholder:text-zink-200"
                            placeholder="Kode Sektor" required>
                    </div>
                    <div class="xl:col-span-12">
                        <label for="nama_sektor" class="inline-block mb-2 text-base font-medium">Nama Sektor</label>
                        <input type="text" id="nama_sektor" name="nama_sektor"
                            class="form-input border-slate-200 dark:border-zink-500 focus:outline-none focus:border-custom-500 disabled:bg-slate-100 dark:disabled:bg-zink-600 disabled:border-slate-300 dark:disabled:border-zink-500 dark:disabled:text-zink-200 disabled:text-slate-500 dark:text-zink-100 dark:bg-zink-700 dark:focus:border-custom-800 placeholder:text-slate-400 dark:placeholder:text-zink-200"
                            placeholder="Nama Sektor" required>
                    </div>
                    <div class="xl:col-span-12">
                        <label for="alamat_sektor" class="inline-block mb-2 text-base font-medium">Alamat</label>
                        <input type="text" id="alamat_sektor" name="alamat_sektor"
                            class="form-input border-slate-200 dark:border-zink-500 focus:outline-none focus:border-custom-500 disabled:bg-slate-100 dark:disabled:bg-zink-600 disabled:border-slate-300 dark:disabled:border-zink-500 dark:disabled:text-zink-200 disabled:text-slate-500 dark:text-zink-100 dark:bg-zink-700 dark:focus:border-custom-800 placeholder:text-slate-400 dark:placeholder:text-zink-200"
                    ...
                    <div class="xl:col-span-6">
                        <label for="joiningDateInput" class="inline-block mb-2 text-base font-medium">Tanggal
                            Beridiri</label>
                        <input type="text" id="joiningDateInput" name="tgl_berdiri"
                            class="form-input border-slate-200 dark:border-zink-500 focus:outline-none focus:border-custom-500 disabled:bg-slate-100 dark:disabled:bg-zink-600 disabled:border-slate-300 dark:disabled:border-zink-500 dark:disabled:text-zink-200 disabled:text-slate-500 dark:text-zink-100 dark:bg-zink-700 dark:focus:border-custom-800 placeholder:text-slate-400 dark:placeholder:text-zink-200"
                            placeholder="Pilih Tanggal" data-provider="flatpickr" data-date-format="d M, Y">
                    </div>
                    <div class="xl:col-span-12">
                        <label for="cabang" class="inline-block mb-2 text-base font-medium">Nama Cabang</label>
                        <select
                            class="form-input border-slate-200 dark:border-zink-500 focus:outline-none focus:border-custom-500 disabled:bg-slate-100 dark:disabled:bg-zink-600 disabled:border-slate-300 dark:disabled:border-zink-500 dark:disabled:text-zink-200 disabled:text-slate-500 dark:text-zink-100 dark:bg-zink-700 dark:focus:border-custom-800 placeholder:text-slate-400 dark:placeholder:text-zink-200"
                            name="cabang" id="cabang" required>
